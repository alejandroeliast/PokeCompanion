        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (m_drawAdd)
            {
                ...
                switch (m_keyAdd.GetArrayElementAtIndex(0).propertyType)
                {
                    ...
                    default:
                        EditorGUI.PropertyField(keyRect, m_keyAdd.GetArrayElementAtIndex(0), Style.keyAddContent, true);
                        break;
                }
                ...
                if (GUI.Button(rect, "Add"))
                {
                    ...
                    int index = m_keysValues.arraySize - 1;

                    object newKey = GetPropertyValue(m_keyAdd.GetArrayElementAtIndex(0));
                    
                    int count;
                    if (ContainsId(newKey, index, out count))
                    {
                        // if (count >= 1)
                        //     Remove(index);
                    }
                    ...
                    {
                        SetPropertyValue(m_keysValues.GetArrayElementAtIndex(index), newKey);
                        SetPropertyValue(m_keysProp.GetArrayElementAtIndex(index), newKey);
                    }
            ...
            switch (keyPropType)
            {
                ...
                case SerializedPropertyType.Generic:
                    ...
                    else
                    {
                        ...
                        EditorGUI.PropertyField(new Rect(rect.x + 15, keyRect.y, keyRect.width + 35, keyRect.height), keyToUse, Style.idContent, true);
                    }
                    break;
